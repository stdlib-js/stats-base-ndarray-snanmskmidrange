...
tape( 'the function has an arity of 1', function test( t ) {
	t.strictEqual( snanmskmidrange.length, 1, 'has expected arity' );
	t.end();
});

tape( 'the function calculates the mid-range of a one-dimensional single-precision floating-point ndarray according to a mask, ignoring NaN values', function test( t ) {
	var mask;
	var x;
	var v;
	...
	v = snanmskmidrange( [ vector( x, 'float32', x.length, 1, 0 ), vector( mask, 'uint8', mask.length, 1, 0 ) ] );
...
tape( 'if provided an empty ndarray, the function returns `NaN`', function test( t ) {
	...
	v = snanmskmidrange( [ vector( x, 'float32', 0, 1, 0 ), vector( mask, 'uint8', 0, 1, 0 ) ] );
	t.strictEqual( isnanf( v ), true, 'returns expected value' );
...
tape( 'if provided an ndarray containing a single element, the function returns the element', function test( t ) {
	...
	v = snanmskmidrange( [ vector( x, 'float32', 1, 1, 0 ), vector( mask, 'uint8', 1, 1, 0 ) ] );
...
tape( 'the function supports one-dimensional ndarrays having non-unit strides', function test( t ) {
	...
	v = snanmskmidrange( [ vector( x, 'float32', 5, 2, 0 ), vector( mask, 'uint8', 5, 2, 0 ) ] );
...
tape( 'the function supports one-dimensional ndarrays having negative strides', function test( t ) {
	...
	v = snanmskmidrange( [ vector( x, 'float32', 5, -2, 8 ), vector( mask, 'uint8', 5, -2, 8 ) ] );
...
tape( 'the function supports one-dimensional ndarrays having non-zero offsets', function test( t ) {
	...
	v = snanmskmidrange( [ vector( x, 'float32', 5, 2, 1 ), vector( mask, 'uint8', 5, 2, 1 ) ] );
	t.strictEqual( v, 1.0, 'returns expected value' );
